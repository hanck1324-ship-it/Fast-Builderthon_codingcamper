...
2. Supabase Edge Function: add-tokens
   ...
   export async function addTokens(
     ...
     // 1. token_transactions에 기록
     // 2. profiles.total_tokens 업데이트 (atomic)
     // 3. 업데이트된 총 토큰 반환
   }

3. 프론트엔드 표시:
   - 헤더에 현재 토큰 수 (실시간 업데이트)
   - 토큰 획득 시 "+20" 팝업 애니메이션
   - 토론 종료 시 요약 모달 (획득 토큰, 발언 횟수)
